...
connection.query(
    "CREATE UNIQUE INDEX UNIQUE_ADDRESS_ID_FK_PUBLIC_KEY_ID ON ACCOUNT_IDS_MAINNET (address_id, FK_PUBLIC_KEY_ID, is_deleted);"
)
...
async function softDeleteAllAddressIdsAfterJoiningPublicKey(public_key_string, addressId, deleteReceiptHash) {
    let sql = "UPDATE ACCOUNT_IDS_MAINNET SET is_deleted = 1, delete_receipt_hash = " + "'" + deleteReceiptHash + "'" + " WHERE FK_PUBLIC_KEY_ID = " + "(SELECT id FROM PUBLIC_KEY_MAINNET WHERE public_key_string = " + "'" + public_key_string + "'" + ")" + "AND address_id = " + "'" + addressId + "'" + ";";
    const results = await connection.promise().execute(sql);
...
async function insertAddressIds(publicKeyId, addressId, blockHeight, receiptHash, saveType) {
    const NOT_DELETED = 0;
    let sql = "INSERT INTO ACCOUNT_IDS_MAINNET (FK_PUBLIC_KEY_ID, address_id, block_height, receipt_hash, save_type, is_deleted) VALUES " + "(" + publicKeyId + ", " + "'" + addressId + "', " + blockHeight + ", " + "'" + receiptHash + "', " + "'" + saveType + "', " + NOT_DELETED + ")" + ";";
    const results = await connection.promise().execute(sql);
